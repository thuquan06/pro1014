<?php
/**
 * Sửa Phân công HDV - Modern Interface
 * UC-Assign-Guide: Sửa phân công HDV
 */
...
if (!$assignment) {
    echo '<div class="alert alert-danger">Không tìm thấy phân công</div>';
    return;
}
?>
    ...
    <div class="alert alert-warning">
      ...
      <p style="margin-top: 12px;">
        <label>
          ...
          Vẫn tiếp tục cập nhật (bỏ qua cảnh báo)
        </label>
      </p>
    </div>
  <?php endif; ?>

  <form method="POST" action="?act=admin-assignment-edit&id=<?= $assignment['id'] ?>">
    <div class="form-row">
      <div class="form-group-modern">
        ...
        <select name="id_lich_khoi_hanh" id="id_lich_khoi_hanh" required>
          ...
          <?php foreach ($departurePlans as $plan): ?>
            ...
            $selected = ($assignment['id_lich_khoi_hanh'] == $plan['id']) ? 'selected' : '';
            ?>
      ...
      <div class="form-group-modern">
        ...
        <select name="id_hdv" id="id_hdv" required>
          ...
          <?php foreach ($guides as $guide): ?>
            <?php $selected = ($assignment['id_hdv'] == $guide['id']) ? 'selected' : ''; ?>
    ...
    <div class="form-row">
      ...
      <div class="form-group-modern">
        ...
        <input type="number" name="luong" id="luong" min="0" step="1000" 
               value="<?= safe_html($assignment['luong'] ?? '') ?>" placeholder="Nhập lương">
      </div>
    </div>
    ...
    <div class="form-row">
      <div class="form-group-modern">
        ...
        <input type="date" name="ngay_bat_dau" id="ngay_bat_dau" required 
               value="<?= $assignment['ngay_bat_dau'] ? date('Y-m-d', strtotime($assignment['ngay_bat_dau'])) : '' ?>">
      </div>
      ...
      <div class="form-group-modern">
        ...
        <input type="date" name="ngay_ket_thuc" id="ngay_ket_thuc" required
               value="<?= $assignment['ngay_ket_thuc'] ? date('Y-m-d', strtotime($assignment['ngay_ket_thuc'])) : '' ?>">
      </div>
    </div>

    <div class="form-group-modern full-width">
      <label for="ghi_chu">Ghi chú</label>
      <textarea name="ghi_chu" id="ghi_chu" placeholder="Ghi chú về phân công này..."><?= safe_html($assignment['ghi_chu'] ?? '') ?></textarea>
    </div>
    ...
    <div class="btn-group">
      ...
      <a href="?act=admin-assignments" class="btn btn-secondary">
