<?php
/**
 * Sửa Phân công HDV - Modern Interface với Cảnh báo Trùng lịch
 * UC-Assign-Guide: Cập nhật phân công HDV
 */
...
if (!$assignment) {
    echo '<div style="padding: 20px; background: #fee2e2; color: #991b1b; border-radius: 8px;">Không tìm thấy phân công</div>';
    exit;
}
...
<?php if (!empty($conflictDetails)): ?>
  <div class="conflict-warning">
    ...
    <p style="margin-top: 16px; color: #78350f; font-weight: 600;">
      Bạn có muốn tiếp tục cập nhật không? (Có thể gây xung đột lịch trình)
    </p>
  </div>
<?php endif; ?>

<form method="POST" action="<?= BASE_URL ?>?act=admin-assignment-edit&id=<?= $assignment['id'] ?>">
  <?php if (!empty($conflictDetails)): ?>
  ...
  <div class="form-card">
    ...
    <div class="form-group-modern">
      ...
      <select name="id_hdv" id="id_hdv" required onchange="checkSchedule()">
        ...
        <?php foreach ($guides as $guide): 
          $selected = ($assignment['id_hdv'] == $guide['id']) ? 'selected' : '';
        ?>
    ...
    <div class="form-group-modern">
      <label>Vai trò</label>
      <select name="vai_tro">
        <option value="HDV chính" <?= ($assignment['vai_tro'] == 'HDV chính') ? 'selected' : '' ?>>HDV chính</option>
        <option value="HDV phụ" <?= ($assignment['vai_tro'] == 'HDV phụ') ? 'selected' : '' ?>>HDV phụ</option>
        <option value="Trợ lý" <?= ($assignment['vai_tro'] == 'Trợ lý') ? 'selected' : '' ?>>Trợ lý</option>
      </select>
    </div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
      <div class="form-group-modern">
        ...
        <input type="date" name="ngay_bat_dau" id="ngay_bat_dau" value="<?= $assignment['ngay_bat_dau'] ?>" required onchange="checkSchedule()">
      </div>

      <div class="form-group-modern">
        <label>Ngày kết thúc <span style="color: #ef4444;">*</span></label>
        <input type="date" name="ngay_ket_thuc" id="ngay_ket_thuc" value="<?= $assignment['ngay_ket_thuc'] ?>" required onchange="checkSchedule()">
      </div>
    </div>

    <div class="form-group-modern">
      <label>Lương/Thù lao (VNĐ)</label>
      <input type="number" name="luong" step="0.01" min="0" value="<?= $assignment['luong'] ?? '' ?>" placeholder="VD: 5000000">
    </div>

    <div class="form-group-modern">
      <label>Trạng thái</label>
      <select name="trang_thai">
        <option value="1" <?= ($assignment['trang_thai'] == 1) ? 'selected' : '' ?>>Đã phân công</option>
        <option value="0" <?= ($assignment['trang_thai'] == 0) ? 'selected' : '' ?>>Đã hủy</option>
      </select>
    </div>

    <div class="form-group-modern">
      <label>Ghi chú</label>
      <textarea name="ghi_chu" style="padding: 12px 16px; border: 1px solid var(--border); border-radius: 8px; min-height: 100px;"><?= safe_html($assignment['ghi_chu'] ?? '') ?></textarea>
    </div>
  </div>
  ...
  <div class="form-actions">
    ...
    <?php if (!empty($conflictDetails)): ?>
      <button type="submit" class="btn-warning">
        <i class="fas fa-exclamation-triangle"></i> Vẫn cập nhật (Có trùng lịch)
      </button>
    <?php else: ?>
