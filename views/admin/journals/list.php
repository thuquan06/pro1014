<?php
// views/admin/journals/list.php
?>

<div class="card">
  <div class="card-header">
    <h3><i class="fas fa-book"></i> Nhật ký tour của HDV</h3>
  </div>
  <div class="card-body">
    <!-- Filters -->
    <form method="GET" action="?act=admin-journals" style="margin-bottom: 24px; padding: 20px; background: var(--bg-light); border-radius: 10px;">
      <input type="hidden" name="act" value="admin-journals">
          ...
          <tbody>
            <?php foreach ($journals as $journal): ?>
              <tr>
                ...
                <td>
                  <strong><?= htmlspecialchars($journal['ten_hdv'] ?? 'N/A') ?></strong>
                  <?php if ($journal['email_hdv']): ?>
                    <br><small style="color: var(--text-light);"><?= htmlspecialchars($journal['email_hdv']) ?></small>
                  <?php endif; ?>
                </td>
                <td>
                ...
                <td>
                  <a href="?act=admin-journal-detail&id=<?= $journal['id'] ?>" class="btn btn-sm btn-primary">
